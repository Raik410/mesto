...
const jobInput = formContainer.querySelector(
...
const popupBtnAddCardsClose = popupAddCards.querySelector(
  ".popup__botton-close"
);
...
const descriptionInputValue = popupAddCard.querySelector(
  ".popup__input-name_type_user-job"
);
const submitButtonCreateCardText = popupAddCard.querySelector(
  ".popup__button-text"
);
...
function createCard(name, link) {
  ...
  cardElement
    .querySelector(".card__button-card-detele")
    .addEventListener("click", function (evt) {
      evt.target.closest(".card").remove(); // Удаление карточки
    });
  cardImage.addEventListener("click", function () {
    // Попап для открытия попапа прямо с карточки
...
function handleCardFormSubmit(evt) {
  evt.preventDefault();
  sectionCards.prepend(
    createCard(titleInputValue.value, descriptionInputValue.value)
  );
  closePopup(popupAdd);
  popupFormSubmitAddCard.reset();
  submitButtonCreateCard.setAttribute("disabled", "");
  submitButtonCreateCard.classList.add(validationConfig.inactiveButtonClass);
  submitButtonCreateCardText.classList.add(
    validationConfig.inactiveSubmitButtonSelectorText
  );
}
